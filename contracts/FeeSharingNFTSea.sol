...
contract FeeSharingSystem is ReentrancyGuard, Ownable {
    ...
    /**
     * @notice Constructor
     * @param _NFTSeaToken address of the token staked (NFTS)
     ...
     * @dev There is a limit of 1 NFTS per deposit to prevent potential manipulation of current shares
     */
    function deposit(uint256 amount, bool claimRewardToken) external nonReentrant {
        require(amount >= PRECISION_FACTOR, "Deposit: Amount must be >= 1 NFTS");
        ...
        NFTSeaToken.safeTransferFrom(msg.sender, address(this), amount);
        ...
        // Verify NFTS token allowance and adjust if necessary
        _checkAndAdjustNFTSTokenAllowanceIfRequired(amount, address(tokenDistributor));
    ...
    /**
     * @notice Calculate value of NFTS for a user given a number of shares owned
     * @param user address of the user
     */
     ...
     */
    function _checkAndAdjustNFTSTokenAllowanceIfRequired(uint256 _amount, address _to) internal {
        if (NFTSeaToken.allowance(address(this), _to) < _amount) {
            NFTSeaToken.approve(_to, type(uint256).max);
        }
    }
    ...
    function _withdraw(uint256 shares, bool claimRewardToken) internal {
        ...
        // Retrieve total amount staked and calculated current amount (in NFTS)
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(address(this));
        ...
        // Transfer NFTS tokens to sender
        NFTSeaToken.safeTransfer(msg.sender, currentAmount);
