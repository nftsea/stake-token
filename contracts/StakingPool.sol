 ...
 * @notice It is a staking pool for Uniswap V2 LP tokens (stake Uniswap V2 LP tokens -> get NFTS).
 */
contract StakingPoolForUniswapV2Tokens is Ownable, Pausable, ReentrancyGuard {
    ...
    // NFTS token (token distributed)
    IERC20 public immutable NFTSeaToken;

    // The staked token (i.e., Uniswap V2 WETH/NFTS LP token)
    ...
    // Tokens distributed per block (in NFTSeaToken)
     ...
     * @param _NFTSeaToken reward token address
     * @param _rewardPerBlock reward per block (in NFTS)
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (userInfo[msg.sender].amount >= 0) {
            ...
            if (pendingRewards > 0) {
                NFTSeaToken.safeTransfer(msg.sender, pendingRewards);
            }
        }
     ...
     * @param amount amount to withdraw (in NFTSeaToken)
