 ...
 * @notice It receives NFTSea protocol fees and owns the FeeSharingSystem contract.
...
contract FeeSharingSetter is ReentrancyGuard, AccessControl {
    ...
    constructor(
        ...
        NFTSeaToken = feeSharingSystem.NFTSeaToken();
     ...
     * based on the balance of NFTS in fee-staking addresses that exist in the set.
     */
    function updateRewards() external onlyRole(OPERATOR_ROLE) {
        ...
        if (numberAddressesForFeeStaking > 0) {
            uint256[] memory NFTSBalances = new uint256[](numberAddressesForFeeStaking);
            ...
            for (uint256 i = 0; i < numberAddressesForFeeStaking; i++) {
                uint256 NFTSBalance = NFTSeaToken.balanceOf(_feeStakingAddresses.at(i));
                totalAmountStaked += NFTSBalance;
                NFTSBalances[i] = NFTSBalance;
            }
            ...
            if (totalAmountStaked > 0) {
                ...
                for (uint256 i = 0; i < numberAddressesForFeeStaking; i++) {
                    uint256 amountToTransfer = (NFTSBalances[i] * reward) / totalAmountStaked;
