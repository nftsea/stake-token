 ...
 * @notice It distributes NFTS tokens with parallel rolling Merkle airdrops.
...
contract MultiRewardsDistributor is Pausable, ReentrancyGuard, Ownable {
    ...
    // Time buffer for the admin to withdraw NFTS tokens if the contract becomes paused
    uint256 public constant BUFFER_ADMIN_WITHDRAW = 3 days;

    // Standard safe guard amount (set at 1 NFTS)
    ...
    // Total amount claimed by user (in NFTS)
    ...
    /**
     * @notice Constructor
     * @param _NFTSeaToken address of the NFTSea token
     */
    constructor(address _NFTSeaToken) {
    ...
    /**
     * @notice Transfer NFTS tokens back to owner
