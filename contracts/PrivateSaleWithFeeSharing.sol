 ...
 * @notice It handles the private sale for NFTS tokens (against ETH) and the fee-sharing
 * mechanism for sale participants. It uses a 3-tier system with different
 * costs (in ETH) to participate. The exchange rate is expressed as the price of 1 ETH in NFTS token.
...
contract PrivateSaleWithFeeSharing is Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable NFTSeaToken;
    ...
    // Total NFTS expected to be distributed
    uint256 public immutable TOTAL_NFTS_DISTRIBUTED;

    // Current sale phase (uint8)
    SalePhase public currentPhase;

    // Block where participants can withdraw the NFTS tokens
    uint256 public blockForWithdrawal;

    // Price of WETH in NFTS for the sale
    uint256 public priceOfETHInNFTS;
    ...
    event NewPriceOfETHInNFTS(uint256 price);
    ...
    /**
     * @notice Withdraw NFTS + pending WETH
     */
    function withdraw() external nonReentrant {
        ...
        // Calculate amount of NFTS to transfer based on the tier
        uint256 NFTSAmountToTransfer = allocationCostPerTier[userInfo[msg.sender].tier] * priceOfETHInNFTS;

        // Transfer NFTS token to sender
        NFTSeaToken.safeTransfer(msg.sender, NFTSAmountToTransfer);

        emit Withdraw(msg.sender, userInfo[msg.sender].tier, NFTSAmountToTransfer);
    }
    ...
    /**
     * @notice Update block deadline for withdrawal of NFTS
     * @param _blockForWithdrawal block for withdrawing NFTS for sale participants
     */
    ...
    /**
     * @notice Set price of 1 ETH in NFTS
     * @param _priceOfETHinNFTS price of 1 ETH in NFTS
     */
    function setPriceOfETHInNFTS(uint256 _priceOfETHinNFTS) external onlyOwner {
        require(currentPhase == SalePhase.Pending, "Owner: Phase must be Pending");
        priceOfETHInNFTS = _priceOfETHinNFTS;

        emit NewPriceOfETHInNFTS(_priceOfETHinNFTS);
    }
    ...
    function updateSalePhase(SalePhase _newSalePhase) external onlyOwner {
        if (_newSalePhase == SalePhase.Deposit) {
            ...
            // Risk checks
            require(priceOfETHInNFTS > 0, "Owner: Exchange rate must be > 0");
            require(getMaxAmountNFTSToDistribute() == TOTAL_NFTS_DISTRIBUTED, "Owner: Wrong amount of NFTS");
            require(
                NFTSeaToken.balanceOf(address(this)) >= TOTAL_NFTS_DISTRIBUTED,
                "Owner: Not enough NFTS in the contract"
            );
    ...
    /**
     * @notice Withdraw the total commited amount (in ETH) and any NFTS surplus.
    ...
    function withdrawCommittedAmount() external onlyOwner nonReentrant {
        ...
        // If some tiered users did not participate, transfer the NFTS surplus to contract owner
        if (totalAmountCommitted * priceOfETHInNFTS < (TOTAL_NFTS_DISTRIBUTED)) {
            uint256 tokenAmountToReturnInNFTS = TOTAL_NFTS_DISTRIBUTED - (totalAmountCommitted * priceOfETHInNFTS);
            NFTSeaToken.safeTransfer(msg.sender, tokenAmountToReturnInNFTS);
        }
    ...
    /**
     * @notice Retrieve max amount to distribute (in NFTS) for sale
     */
    function getMaxAmountNFTSToDistribute() public view returns (uint256 maxAmountCollected) {
        for (uint8 i = 1; i <= NUMBER_TIERS; i++) {
            maxAmountCollected += (allocationCostPerTier[i] * numberOfParticipantsForATier[i]);
        }

        return maxAmountCollected * priceOfETHInNFTS;
    }
}
