...
/**
 * @title TokenSplitter
 * @notice It splits NFTS to team/treasury/trading volume reward accounts based on shares.
 */
contract TokenSplitter is Ownable, ReentrancyGuard {
    ...
    // Total NFTS tokens distributed across all accounts
    ...
    /**
     * @notice Release NFTS tokens to the account
    ...
    function releaseTokens(address account) external nonReentrant {
        ...
        uint256 totalTokensReceived = NFTSeaToken.balanceOf(address(this)) + totalTokensDistributed;
        ...
        NFTSeaToken.safeTransfer(account, pendingRewards);
    ...
    /**
     * @notice Retrieve amount of NFTS tokens that can be transferred
     * @param account address of the account
     */
